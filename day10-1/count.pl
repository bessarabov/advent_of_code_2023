...
#     S
    ...
    while (1) {
        ...
        # move point 2 to direction 2
        my ($new_p2_x, $new_p2_y) = move($p2_x, $p2_y, $d2);

        return 0 if $new_p2_x < 0;
        return 0 if $new_p2_y < 0;

        return 0 if $new_p2_x > $MAX_X;
        return 0 if $new_p2_y > $MAX_Y;

        my $p2_new_pipe_id = $field[$new_p2_y]->[$new_p2_x];

        return 0 if $p2_new_pipe_id eq '.';
        return 0 if not are_connected($pipe_id, $p2_new_pipe_id, $d2);

        ($p2_x, $p2_y) = ($new_p2_x, $new_p2_y);
        $d2 = get_new_direction($p2_new_pipe_id, $d2);

        return 1 if $VISITED{$p1_x . '_' . $p1_y};
        return 1 if $VISITED{$p2_x . '_' . $p2_y};

        $VISITED{$p1_x . '_' . $p1_y} = 1;
        $VISITED{$p2_x . '_' . $p2_y} = 1;
